...
def crossingDiagonals():
    ...
    if not (b == p or a / b == 0.5):
        ...
        print(
            f'Crease the line between the left side at {m}/{p_m} to the right side at {"the corner" if n == 0 else f"{n}/{p_n}"}'
        )
...
if not (tempA == a and tempB == b):
    print(f"Fraction changed to {a}/{b}")
...
while x % 2 == 0 and x > 0:
    x //= 2
    p //= 2
xBin = f"{x:0{int(math.log2(p))}b}" if not (x / p == 1) else ""
print(f"Folding mark {x}/{p} on left side")
for char in xBin[::-1]:
    print(f'Fold the left side {"down" if char == "1" else "up"} to most recent point')
